...
const clean: LaunchResult = { code: 0, signal: null };
...
describe('runLoop', () => {
  ...
  test('consumes a switch sentinel and relaunches into the target', async () => {
    ...
    await runLoop({
      ...
      launch: async (p) => {
        launched.push(p.id);
        if (p.id === 'work') writeSentinel(ctx.paths, 'home'); // simulate in-session /switch
        return clean;
      },
    });
  ...
  test('ignores a switch to an unknown profile and stops', async () => {
    ...
    await runLoop({
      ...
      launch: async (p) => {
        launched.push(p.id);
        if (p.id === 'work') writeSentinel(ctx.paths, 'ghost');
        return clean;
      },
    });
    assert.deepEqual(launched, ['work']);
  ...
  test('throws when no active profile is set', async () => {
    await assert.rejects(
      () => runLoop({ paths: ctx.paths, registry: reg, launch: async () => clean }),
  ...
  test('refuses to start when a live run lock is held', async () => {
    ...
    await assert.rejects(
      () => runLoop({ paths: ctx.paths, registry: reg, maxIterations: 1, launch: async () => clean }),
      RunLockError,
    );
  });
});
