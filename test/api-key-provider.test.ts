...
import type { LaunchContext } from '../src/auth/provider.js';
...
describe('ApiKeyProvider', () => {
  ...
  const launchCtx = (configDir: string): LaunchContext => ({ env: {}, configDir, args: [] });
  ...
  test('settings.json gets apiKeyHelper pointing at the helper script', async () => {
    ...
    const settings = readJson<{ apiKeyHelper: string }>(join(p.configDir, 'settings.json'));
  ...
  test('merging preserves pre-existing settings keys', async () => {
    ...
    await provider.authenticate(p, { secret: 'sk-ant-xyz' });
    await provider.applyTo(launchCtx(p.configDir), p);
    const settings = readJson<Record<string, unknown>>(join(p.configDir, 'settings.json'));
