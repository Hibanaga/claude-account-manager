...
import { type AuthKind, type Profile, Registry } from './core/registry.js';
...
import { Launcher, type LaunchResult, buildLaunchContext, launchClaudeTool } from './launcher.js';
...
/** Injectable IO/side-effect seams so `add`/`switch` are testable without a TTY. */
export interface AddDeps {
  isInteractive: () => boolean;
  promptLine: (msg: string) => Promise<string>;
  launchSetupToken: (bin: string, configDir: string, env: NodeJS.ProcessEnv) => Promise<LaunchResult>;
}
...
export async function cmdAdd(
  ...
  if (!name) throw new CamError('Usage: cam add <name> [--api-key-stdin | --oauth-token-stdin]');
  assertValidProfileId(name);
  const id = name;
  if (app.registry.has(id)) throw new CamError(`Profile "${id}" already exists.`);

  const authKind: AuthKind = values['api-key-stdin'] ? 'api-key' : 'subscription-oauth';
  ...
  if (authKind === 'api-key') {
    ...
    if (res.code !== 0) throw new CamError('claude setup-token failed; aborting add.');
  ...
  if (!secret) throw new CamError('No credential provided.');
...
function printList(app: App): void {
...
function resolveTarget(app: App, token: string): Profile {
  if (/^\d+$/.test(token)) {
    const profile = app.registry.list()[Number.parseInt(token, 10) - 1];
    if (!profile) throw new CamError(`No account at number ${token}. Run "cam list".`);
    return profile;
  }
  return app.registry.getOrThrow(token);
}

function cmdCurrent(app: App): void {
  const active = app.registry.getActive();
  process.stdout.write(active ? `${active.id} (${active.authKind})\n` : 'No active account.\n');
}

export function cmdStatus(app: App, env: NodeJS.ProcessEnv): void {
  const active = app.registry.getActive();
  ...
  if (env.CAM_RUN_LOOP === '1') {
    ...
    process.stdout.write(
      "✗ not in a 'cam run' session — /switch stages a switch but nothing relaunches.\n" +
        "  Start sessions with 'cam run' (tip: alias claude='cam run').\n",
    );
  }
}

async function cmdUse(app: App, env: NodeJS.ProcessEnv, rest: string[]): Promise<never> {
  const name = rest[0];
  if (!name) throw new CamError('Usage: cam use <name> [args…]');
...
async function cmdRun(app: App, env: NodeJS.ProcessEnv): Promise<void> {
...
export async function cmdSwitch(
  ...
  let token = rest[0];
  if (!token) {
    if (!deps.isInteractive()) throw new CamError('Usage: cam switch <name|number>');
    if (app.registry.list().length === 0) throw new CamError('No accounts to switch to.');
    printList(app);
  ...
  if (!deps.inRunLoop()) {
    process.stdout.write(
      "⚠ You're not in a 'cam run' session — this won't take effect until you launch one.\n",
    );
  }
}

function cmdRemove(app: App, rest: string[]): void {
  const name = rest[0];
  if (!name) throw new CamError('Usage: cam remove <name>');
  const profile = app.registry.getOrThrow(name);
  ...
  if (pathExists(profile.configDir)) rmSync(profile.configDir, { recursive: true, force: true });
